...
@Component({
  ...
  styleUrls: ['./role.component.scss'], standalone: true,
...
export class RoleComponent extends BaseComponent implements OnInit {
  ...
  async create(name: HTMLInputElement) {
    if (name.value.length > 4) {
      ...
      await this.roleService.create(name.value, () => {
      ...
      }).then(async () => await this.getRoles()).then(() => this.hideSpinner(SpinnerType.BallAtom));
    } else {
  ...
  async editRole(name: HTMLInputElement) {
    ...
    await this.roleService.updateRole(this.id, name.value).then(async () => await this.getRoles())
  }
}
