...
@Component({
  selector: 'app-cargo',
  standalone: true,
  imports: [CommonModule, RouterModule, MatDialogModule, DeleteDirectiveModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatButtonModule, FormsModule, MatTableModule, MatPaginatorModule],
...
export class ShipmentComponent  implements OnInit {
  constructor( private fb: FormBuilder, private shipmentService: ShipmentService) {
    this.shipperForm = this.fb.group({
      ...
      freight: [null, [Validators.required, Validators.minLength(2), Validators.maxLength(4)]],
    })
  }
  ...
  async upShip(element: GetAllShippers) {
    this.shipValue = ''
    this.updMode = false
    await this.shipmentService.update(element).then(r => this.getAllShipments())
    this.shipperForm.reset()
  }
  async getAllShipments() {
    await this.shipmentService.getAllShipments().then(res => this.dataSource = res.shippers)
  }
